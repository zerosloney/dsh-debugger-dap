...
export interface DebugArgs {
  ...
  target_line?: number
  target_id?: number
  data_breakpoints?: Array<{ data_id?: string; address?: string; name?: string; access_type?: 'read' | 'write' | 'readWrite'; condition?: string; hit_condition?: string }>
...
export async function runDebugAction(
  ...
  switch (args.action) {
    ...
    case 'set_function_breakpoints': {
      ...
      const session = manager.sessionFor(owner, args.session_id)
      const breakpoints = await session.setFunctionBreakpoints(
        args.functions.map(name => ({ name, condition: args.condition, hitCondition: args.hit_condition })),
        signal,
      )
    ...
    case 'pause': {
      const session = manager.sessionFor(owner, args.session_id)
      const command: 'continue' | 'next' | 'stepIn' | 'stepOut' | 'reverseContinue' | 'pause' =
    ...
    case 'variables': {
      if (args.variables_ref === undefined) {
        throw new DebugError(
          'invalid_arguments',
          "action 'variables' requires 'variables_ref' from a scopes, variables, or evaluate result.",
        )
      }
      const session = manager.sessionFor(owner, args.session_id)
    ...
    case 'disconnect': {
      ...
      if (snapshot === undefined) return { action: 'disconnect', sessions: manager.list(owner) }
...
function requireOwner(agent: unknown): object {
