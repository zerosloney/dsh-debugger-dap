...
export function encodeMessage(message: unknown): Buffer<ArrayBufferLike> {
...
export class MessageDecoder {
  ...
  private tryTake(): unknown | undefined {
    ...
    if (contentLength < 0) throw new FramingError('DAP frame header is missing Content-Length', headerBlock)
    ...
    if (this.buffer.length < bodyStart + contentLength) return undefined
