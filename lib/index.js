...
import { AdapterUnavailableError, createAutoInstallingResolver, resolveAdapter, } from './adapters.js';
...
export const Config = z.object({
    ...
    autoInstallAdapters: z
        .boolean()
        .default(false)
        .description("When launch/attach cannot resolve an adapter, try installing it once before failing: debugpy via pip, dlv via go install (Go toolchain required), netcoredbg from GitHub releases. Installed binaries land in ~/.dsh-debugger-dap/adapters and are probed automatically. The install_adapter action works regardless of this switch."),
    installTimeoutMs: z
        .natural()
        .min(10_000)
        .default(600_000)
    ...
    ledgerPath: z
        .string()
        .default('')
        .description('Session trace ledger (JSONL) path; empty = ~/.dsh-debugger-dap/ledger.jsonl. Appends one line per key debug event (session start/end, breakpoint set/hit, exception, stop, request errors) for later forensics.'),
    ledgerMaxBytes: z
        .natural()
        .min(1024)
        .default(5 * 1024 * 1024)
