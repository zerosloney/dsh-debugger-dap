...
const BUILT_IN_RECIPES = [
    ...
    },
    {
        ...
        fixedArgs: [],
        installHint: "adapter 'lldb-dap' is not available: install the LLVM DAP binary (llvm-dap, lldb-dap, or dap-server depending on your LLVM version) and ensure it is on PATH. On macOS with Xcode, you may need to build from https://llvm.org/git/dap.",
    },
    {
        id: 'js-debug',
        probeCommands: ['node'],
        fixedArgs: [],
        installHint: "adapter 'js-debug' is not available: run 'npm install -g @vscode/js-debug' and ensure 'node' is on PATH. js-debug is also bundled with VS Code and the 'nodedebug' adapter.",
...
function guessAdapterId(program) {
    ...
    if (program.endsWith('.dll') || program.endsWith('.exe'))
        return 'netcoredbg';
    if (program.endsWith('.c') || program.endsWith('.cpp') || program.endsWith('.cxx') || program.endsWith('.h') || program.endsWith('.hpp'))
        return 'lldb-dap';
    if (program.endsWith('.rs'))
        return 'codelldb';
    if (program.endsWith('.js') || program.endsWith('.mjs') || program.endsWith('.cjs') || program.endsWith('.ts') || program.endsWith('.mts') || program.endsWith('.cts'))
        return 'js-debug';
    return undefined;
}
/**
