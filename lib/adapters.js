...
import { INSTALLABLE_ADAPTERS, installAdapter, managedBinDirs } from './install.js';
/**
 * Expand a user-home prefix (`~`) and environment variables (`%VAR%`,
 * `${VAR}`) in a path.
 */
export function expandPath(str) {
    if (str === '~')
        return homedir();
    let result = str;
...
function parseSemverParts(name, prefix) {
 ...
 */
export function findVsCodeExtensionEntry(extensionPrefix, relativeCandidates) {
    const home = homedir();
    const searchRoots = [
    ...
    if (process.platform === 'win32') {
        if (process.env.LOCALAPPDATA) {
            searchRoots.push(join(process.env.LOCALAPPDATA, 'Programs', 'Microsoft VS Code', 'resources', 'app', 'extensions'), join(process.env.LOCALAPPDATA, 'Programs', 'Microsoft VS Code Insiders', 'resources', 'app', 'extensions'));
        }
        if (process.env.PROGRAMFILES) {
            searchRoots.push(join(process.env.PROGRAMFILES, 'Microsoft VS Code', 'resources', 'app', 'extensions'));
        }
    }
    else if (process.platform === 'darwin') {
        searchRoots.push('/Applications/Visual Studio Code.app/Contents/Resources/app/extensions', '/Applications/Visual Studio Code - Insiders.app/Contents/Resources/app/extensions');
    }
    else {
        searchRoots.push('/usr/share/code/resources/app/extensions', '/usr/share/code-insiders/resources/app/extensions', '/snap/code/current/usr/share/code/resources/app/extensions');
    }
...
const BUILT_IN_RECIPES = [
    ...
    },
    {
        ...
        fixedArgs: ['--port', '0'],
        installHint: "adapter 'codelldb' is not available: install the CodeLLDB extension in VS Code ('vadimcn.vscode-lldb') or ensure the codelldb binary is on PATH.",
        transport: 'tcp',
    },
];
/**
...
export function resolveAdapter(options, adapterConfig, commandExists = defaultCommandExists, findExtension = findVsCodeExtensionEntry) {
    ...
    if (recipe === undefined) {
        if (wanted === 'js-debug') {
            ...
            throw new AdapterUnavailableError("adapter 'js-debug' has no built-in command: it is a VS Code-bundled TCP DAP server script. Install the JavaScript Debugger extension in VS Code ('ms-vscode.js-debug') for automatic discovery, or declare it in the plugin's 'adapters' config (e.g. adapters: { 'js-debug': { command: 'node', args: ['~/.vscode/extensions/ms-vscode.js-debug/dist/src/dapDebugServer.js'], transport: 'tcp', connectPort: 12722 } }).");
        }
    ...
    if (wanted === 'codelldb') {
        const discovered = findExtension('vadimcn.vscode-lldb', process.platform === 'win32' ? ['adapter/codelldb.exe'] : ['adapter/codelldb']);
...
function expandConfigEntry(entry) {
    return {
        ...
        args: entry.args?.map(expandPath) ?? [],
        env: entry.env,
 ...
 */
export function createAutoInstallingResolver(adapterConfig, options) {
    return async (resolveOptions) => {
        const attempt = () => resolveAdapter(resolveOptions, adapterConfig, options.commandExists ?? defaultCommandExists);
        try {
            return attempt();
        }
        catch (error) {
            if (!options.autoInstall || !(error instanceof AdapterUnavailableError))
                throw error;
            const wanted = resolveOptions.adapter ?? guessAdapterId(resolveOptions.program);
            if (wanted === undefined || !INSTALLABLE_ADAPTERS.includes(wanted))
                throw error;
            try {
            ...
            try {
                ...
                throw new AdapterUnavailableError(`'${wanted}' was installed but still does not resolve: ${detail}\nThe managed adapter directory may need a host restart to be picked up.`);
            }
        }
    };
}
...
export function readLaunchConfigurations(workspaceDir) {
...
export function resolveVsCodeVariables(value, workspaceDir, activeFile) {
    if (typeof value === 'string') {
        ...
        return (isPath ? normalize(expanded) : expanded);
    }
    if (Array.isArray(value)) {
        return value.map(item => resolveVsCodeVariables(item, workspaceDir, activeFile));
    }
...
export function mapVsCodeTypeToAdapter(type) {
...
export function readTasksConfigurations(workspaceDir) {
...
export function resolveTaskCommand(taskLabel, workspaceDir) {
    const tasks = readTasksConfigurations(workspaceDir);
    const target = taskLabel.trim().toLowerCase();
    const found = tasks.find(t => (typeof t.label === 'string' && t.label.trim().toLowerCase() === target) ||
    ...
    let args = Array.isArray(resolved.args) ? resolved.args.map(String) : [];
    let shell = true;
    if (!command) {
...
export async function runPreLaunchTask(taskLabel, workspaceDir, signal, timeoutMs = 60_000) {
...
export function resolveLaunchConfig(options) {
    ...
    const program = typeof resolved.program === 'string'
        ? resolved.program
    ...
    const env = typeof resolved.env === 'object' && resolved.env !== null ? resolved.env : undefined;
    const stopOnEntry = typeof resolved.stopOnEntry === 'boolean'
        ? resolved.stopOnEntry
        : typeof resolved.stopAtEntry === 'boolean'
            ? resolved.stopAtEntry
            : undefined;
    const processId = typeof resolved.processId === 'number'
        ? resolved.processId
