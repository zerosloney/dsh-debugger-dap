    ...
    switch (args.action) {
        case 'launch': {
            ...
            if (args.launch_config !== undefined || program === undefined || program.length === 0) {
                ...
                if (resolved) {
                    if (!program && resolved.program)
                        program = resolved.program;
                    if (!adapterId && resolved.adapter)
                        adapterId = resolved.adapter;
                    if (!launchArgs && resolved.args)
                        launchArgs = resolved.args;
                    if (!cwd && resolved.cwd)
                        cwd = resolved.cwd;
                    if (stopOnEntry === undefined && resolved.stopOnEntry !== undefined)
                        stopOnEntry = resolved.stopOnEntry;
                    if (resolved.env)
                        env = resolved.env;
                    if (resolved.preLaunchTask)
                        preLaunchTask = resolved.preLaunchTask;
                    if (Object.keys(resolved.extraLaunchArgs).length > 0)
                        extraLaunchArgs = resolved.extraLaunchArgs;
                }
            }
            if (preLaunchTask) {
                try {
            ...
            if (resolved?.request === 'attach') {
                if (resolved.processId === undefined) {
                    throw new DebugError('invalid_arguments', `Configuration '${resolved.name}' is an attach configuration but carries no usable processId (VS Code command placeholders cannot be resolved). Pass process_id and use action "attach".`);
                }
                if (adapterId === undefined || adapterId.length === 0) {
                    throw new DebugError('invalid_arguments', `Attach configuration '${resolved.name}' needs a resolvable 'type' in .vscode/launch.json, or pass 'adapter'.`);
                }
        ...
        case 'install_adapter': {
            ...
            if (!INSTALLABLE_ADAPTERS.includes(args.adapter)) {
                throw new DebugError('invalid_arguments', `Adapter '${args.adapter}' cannot be auto-installed. Supported: ${INSTALLABLE_ADAPTERS.join(', ')}; declare others in the 'adapters' plugin config instead.`);
            }
        ...
        case 'ledger': {
            ...
            if (args.ledger_kinds !== undefined) {
                ...
                const valid = new Set(LEDGER_KINDS);
                const invalid = requested.filter(kind => !valid.has(kind));
                if (invalid.length > 0) {
                    throw new DebugError('invalid_arguments', `Unknown ledger kind(s): ${invalid.join(', ')}. Valid kinds: ${LEDGER_KINDS.join(', ')}.`);
                }
        ...
        case 'set_data_breakpoints': {
            ...
            const shorthand = explicit.length === 0 && (args.address !== undefined || args.watch_name !== undefined || args.data_id !== undefined || args.name !== undefined)
                ? [{
                        dataId: args.data_id,
        ...
        case 'disassemble': {
            ...
            const session = manager.sessionFor(owner, args.session_id);
            const instructions = await session.disassemble(args.memory_reference, Math.max(1, Math.min(args.instruction_count ?? 20, 2000)), {
                offset: args.offset,
